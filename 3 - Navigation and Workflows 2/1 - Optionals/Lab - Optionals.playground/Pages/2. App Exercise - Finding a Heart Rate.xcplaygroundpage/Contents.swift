 ...
 */
let olderHeartRates = [70, 75, 72]
var hrAverage: Int

hrAverage = (olderHeartRates.reduce(0, +) + (heartRate ?? 0)) / (olderHeartRates.count + (heartRate != nil ? 1 : 0))
print(hrAverage)
...
if let currentHR = heartRate {
    hrAverage = (olderHeartRates.reduce(0, +) + currentHR) / (olderHeartRates.count + 1)
} else {
    hrAverage = olderHeartRates.reduce(0, +) / olderHeartRates.count
}
print(hrAverage)
